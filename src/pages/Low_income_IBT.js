...
function IBT() {
    ...
    const IBT_Model = input => {
        ...
        if (quotient >= 3 && remainder !== 0) {
            if (remainder < 10) {
                price += remainder * prices[1];
            }
            price += (prices[2] * remainder)
        }
    ...
    const GetConsumption = () => {
        ...
        console.log("this is cons:", consumption_month)
        ...
        if (quotient >= 3) {
            let textBox = document.getElementById(`textInBox2`);
            let final = parseFloat((quotient - 3) * 100 + remainder).toFixed(2);
            textBox.innerHTML = `${final} KWh`;
        }
